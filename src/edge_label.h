#ifndef BRAVE_DD_Edge_LABEL_H
#define BRAVE_DD_Edge_LABEL_H
...
class BRAVE_DD::EdgeLabel {
    ...
    /*-------------------------------------------------------------*/
        ...
        inline int getIntValue() const {
            BRAVE_DD_DCASSERT(isValueInt());
            return intValue;
        }
        inline long getLongValue() const {
            BRAVE_DD_DCASSERT(isValueLong());
            return longValue;
        }
        inline float getFloatValue() const {
            BRAVE_DD_DCASSERT(isValueFloat());
            return floatValue;
        }
        inline double getDoubleValue() const {
            BRAVE_DD_DCASSERT(isValueDouble());
            return doubleValue;
        }
        //
        // Reduction rules
        ...
        inline void setRule(ReductionRule r) {
            valueType = ValueType::VOID;
            if (r < RULE_EL0 || r > RULE_I1) {
                throw error(BRAVE_DD::ErrCode::VALUE_OVERFLOW, __FILE__, __LINE__);
            }
            rule = r;
        }
        //
