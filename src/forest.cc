    ...
    * ================================================================================================*/
    if (!setting.isRelation() && (em == TERMINAL)) {
        ...
        * --------------------------------------------------------------------------------------------*/
            ...
            if ((child[0].getNodeLevel() == child[1].getNodeLevel())
            ...
            } else if (setting.hasReductionRule(RULE_OR)) {
                Edge zeroOne = getChildEdge(child[0].getNodeLevel(), child[0].getNodeHandle(), !child[0].getSwap(0));
                Edge zeroZero = getChildEdge(child[0].getNodeLevel(), child[0].getNodeHandle(), child[0].getSwap(0));

                bool first = (nodeLevel == child[0].getNodeLevel() + 1) || (child[0].getSwap(0) && child[0].getRule() == RULE_AND) || (!child[0].getSwap(0) && child[0].getRule() == RULE_OR);
                bool second = (child[1].getRule() == RULE_X) && (nodeLevel > child[1].getNodeLevel() + 1);
                bool third = (child[1].getNodeHandle() == zeroOne.getNodeHandle() && child[1].getNodeLevel() == zeroOne.getNodeLevel());
                bool fourth = child[1].getComp() == (child[0].getComp() ^ zeroOne.getComp());
                bool fifth = zeroOne.getRule() == RULE_X;
                bool sixth = child[1].getSwap(0) == zeroOne.getSwap(0);

                if (first && second && third && fourth && fifth && sixth)
                {
                    reduced = child[0];
                    reduced.setRule(RULE_OR);
                    if (child[0].getSwap(0))
                        reduced.setRule(RULE_AND);

                    Edge c0 = getChildEdge(reduced.getNodeLevel(), reduced.getNodeHandle(), reduced.getSwap(0));
                    Edge c1 = getChildEdge(reduced.getNodeLevel(), reduced.getNodeHandle(), !reduced.getSwap(0));

                    if (c1.isConstantOne())
                    {
                        EdgeLabel label1 = 0;
                        packRule(label1, RULE_EH1);
                        Edge res = mergeEdge(nodeLevel, reduced.getNodeLevel() - 1, label1, c0);
                        res.setComp(reduced.getComp() ^ c0.getComp());
                        return res;
                    }

                    if (c1.isConstantZero())
                    {
                        EdgeLabel label1 = 0;
                        packRule(label1, RULE_EH0);
                        Edge res = mergeEdge(nodeLevel, reduced.getNodeLevel() - 1, label1, c0);
                        res.setComp(reduced.getComp() ^ c0.getComp());
                        return res;
                    }
                    ...
                    {
                        EdgeLabel label1 = 0;
                        packRule(label1, RULE_AL1);
                        Edge res = mergeEdge(nodeLevel, reduced.getNodeLevel() - 1, label1, c1);
                        res.setComp(reduced.getComp() ^ c1.getComp());
                        return res;
                    }

                    if (c0.isConstantZero())
                    {
                        EdgeLabel label1 = 0;
                        packRule(label1, RULE_AL0);
                        Edge res = mergeEdge(nodeLevel, reduced.getNodeLevel() - 1, label1, c1);
                        res.setComp(reduced.getComp() ^ c1.getComp());
                        return res;
                    }
                ...
                {
                    Edge oneZero = getChildEdge(child[1].getNodeLevel(), child[1].getNodeHandle(), 0);
                    Edge oneOne = getChildEdge(child[1].getNodeLevel(), child[1].getNodeHandle(), 1);
                    first = child[1].getNodeLevel() == child[0].getNodeLevel();
                    second = (nodeLevel == child[0].getNodeLevel() + 1) || (child[0].getSwap(0) && child[0].getRule() == RULE_AND) || (!child[0].getSwap(0) && child[0].getRule() == RULE_OR);
                    third = child[1].getRule() == RULE_X;
                    fourth = oneZero.getEdgeHandle() == oneOne.getEdgeHandle();
                    fifth = (zeroOne.getNodeLevel() == oneZero.getNodeLevel()) && (zeroOne.getNodeHandle() == oneZero.getNodeHandle());
                    sixth = (zeroOne.getRule() == oneZero.getRule());
                    bool seventh = child[1].getComp() == (child[0].getComp() ^ zeroOne.getComp());
                    bool eighth = (zeroOne.getSwap(0) == oneZero.getSwap(0));
            ...
            } else if (setting.hasReductionRule(RULE_AND)) {
                Edge oneZero = getChildEdge(child[1].getNodeLevel(), child[1].getNodeHandle(), child[1].getSwap(0));
                Edge oneOne = getChildEdge(child[1].getNodeLevel(), child[1].getNodeHandle(), !child[1].getSwap(0));

                if ((oneZero.getNodeLevel() == 0) && (oneOne.getNodeLevel() == 0))
                {
                    std::cout << "Reduce Node: we are in AND pattern and the target has both children for terminal" << std::endl;
                    exit(0);
                }
                bool first = (nodeLevel == child[1].getNodeLevel() + 1) || (!child[1].getSwap(0) && child[1].getRule() == RULE_AND) || (child[1].getSwap(0) && child[1].getRule() == RULE_OR);
                bool second = (child[0].getRule() == RULE_X) && (nodeLevel > child[0].getNodeLevel() + 1);
                bool third = (child[0].getNodeLevel() == oneZero.getNodeLevel()) && (child[0].getNodeHandle() == oneZero.getNodeHandle());
                bool fourth = child[0].getComp() == (child[1].getComp() ^ oneZero.getComp());
                bool fifth = oneZero.getRule() == RULE_X;
                bool sixth = child[0].getSwap(0) == oneZero.getSwap(0);

                if (first && second && third && fourth && fifth && sixth)
                {
                    reduced = child[1];
                    reduced.setRule(RULE_AND);
                    if (child[1].getSwap(0))
                        reduced.setRule(RULE_OR);
                    Edge c0 = getChildEdge(reduced.getNodeLevel(), reduced.getNodeHandle(), reduced.getSwap(0));
                    Edge c1 = getChildEdge(reduced.getNodeLevel(), reduced.getNodeHandle(), !reduced.getSwap(0));
                    ...
                    {
                        EdgeLabel label1 = 0;
                        packRule(label1, RULE_EL1);
                        Edge res = mergeEdge(nodeLevel, reduced.getNodeLevel(), label1, c1);
                        res.setComp(reduced.getComp() ^ c1.getComp());
                        return res;
                    }
                    ...
                    {
                        EdgeLabel label1 = 0;
                        packRule(label1, RULE_AH1);
                        Edge res = mergeEdge(nodeLevel, reduced.getNodeLevel(), label1, c0);
                        res.setComp(reduced.getComp() ^ c0.getComp());
                        return res;
                    }
                ...
                {
                    Edge zeroZero = getChildEdge(child[0].getNodeLevel(), child[0].getNodeHandle(), 0);
                    Edge zeroOne = getChildEdge(child[0].getNodeLevel(), child[0].getNodeHandle(), 1);

                    first = child[0].getNodeLevel() == child[1].getNodeLevel();
                    second = (nodeLevel == child[1].getNodeLevel() + 1) || (!child[1].getSwap(0) && child[1].getRule() == RULE_AND) || (child[1].getSwap(0) && child[1].getRule() == RULE_OR);
                    third = child[0].getRule() == RULE_X;
                    fourth = zeroZero.getEdgeHandle() == zeroOne.getEdgeHandle();
                    fifth = (oneZero.getNodeHandle() == zeroZero.getNodeHandle()) && (oneZero.getNodeLevel() == zeroZero.getNodeLevel());
                    bool sixth = oneZero.getRule() == zeroZero.getRule();
                    bool seventh = child[0].getComp() == (child[1].getComp() ^ oneZero.getComp());

                    if (first && second && third && fourth && fifth && sixth && seventh)
                    {
                        reduced = child[1];
                        reduced.setRule(RULE_AND);
                        if (reduced.getSwap(0))
                        {
                            reduced.setRule(RULE_OR);
                        }
                        Edge c0 = getChildEdge(reduced.getNodeLevel(), reduced.getNodeHandle(), reduced.getSwap(0));
...
Edge Forest::mergeEdge(const Level beginLevel, const Level mergeLevel, const EdgeLabel label, const Edge& reduced, const Value& value)
{
    ...
    if (unpackSwap(label)){
        std::cout << "\nMERGE EDGE: swap in label\n" << std::endl;
        exit(0);
    }

    if (unpackComp(label)){
        std::cout << "\nMergeEdge comp in label\n" << std::endl;
        exit(0);
    }
    /* ---------------------------------------------------------------------------------------------
    * AND OR Merge Edge
    * --------------------------------------------------------------------------------------------*/
   if (setting.hasReductionRule(RULE_AND) && setting.hasReductionRule(RULE_OR)) {
       bool reducedSwap = reduced.getSwap(0);
       if (incomingSkip == 0) {
       ...
       if (reducedSkip == 0) {
           ...
           if (merged.getSwap(0) && merged.getRule() == RULE_OR)
               merged.setRule(RULE_AND);
           else if (merged.getSwap(0) && merged.getRule() == RULE_AND)
               merged.setRule(RULE_OR);
           return normalizeEdge(beginLevel, merged);
       }

       // Here incomingSkip > 0 and redcuedSkip > 0
       if ((reducedRule == RULE_X) && ((incomingRule == RULE_AND) || (incomingRule == RULE_OR))) {
           return normalizeEdge(beginLevel, reduced);
       }

       Edge normalized;
       std::vector<Edge> child(2);
       EdgeLabel mergeLabel = label;
       if ((incomingRule == RULE_X) && ((reducedRule == RULE_OR) || (reducedRule == RULE_AND))) {
           child[0] = reduced;
           child[1] = reduced;
       ...
       if (incomingRule == RULE_OR) {
           if (reducedRule == RULE_OR && !reducedSwap)
               return normalizeEdge(beginLevel, reduced);
           if (reducedRule == RULE_AND && reducedSwap)
               return normalizeEdge(beginLevel, reduced);
           if (isRuleEH(reducedRule) || isRuleAL(reducedRule))
               return normalizeEdge(beginLevel, reduced);
           if ((reducedRule == RULE_OR && reducedSwap) || (reducedRule == RULE_AND && !reducedSwap) || (isRuleEL(reducedRule)) || (isRuleAH(reducedRule)))
           {
               EdgeLabel root1 = 0;
               packRule(root1, RULE_X);
               child[0] = reduced;
               child[1] = mergeEdge(beginLevel, mergeLevel - 1, root1, reduced);
               normalized = reduceEdge(beginLevel, mergeLabel, mergeLevel + 1, child);
               return normalized;
           }
       }

       if (incomingRule == RULE_AND) {
           if (reducedRule == RULE_OR && reducedSwap)
               return normalizeEdge(beginLevel, reduced);
           if (reducedRule == RULE_AND && !reducedSwap)
               return normalizeEdge(beginLevel, reduced);
           if (isRuleEL(reducedRule) || isRuleAH(reducedRule))
               return normalizeEdge(beginLevel, reduced);
           if ((reducedRule == RULE_OR && !reducedSwap) || (reducedRule == RULE_AND && reducedSwap) || (isRuleEH(reducedRule)) || (isRuleAL(reducedRule)))
           {
               EdgeLabel root1 = 0;
               packRule(root1, RULE_X);
               child[1] = reduced;

               child[0] = mergeEdge(beginLevel, mergeLevel - 1, root1, reduced);
               normalized = reduceEdge(beginLevel, mergeLabel, mergeLevel + 1, child);
               return normalized;
           }
       }

       if (isRuleEL(incomingRule) && ((reducedRule == RULE_AND) || (reducedRule == RULE_OR))) {
           child[1] = reduced;
           child[1].setComp(reduced.getComp());
           child[0].setRule(RULE_X);
           if (incomingRule == RULE_EL0) {
       ...
       if (isRuleEH(incomingRule) && ((reducedRule == RULE_AND) || (reducedRule == RULE_OR)))
       {
           child[0] = reduced;
           child[0].setComp(reduced.getComp());
       ...
       if (isRuleAL(incomingRule) && ((reducedRule == RULE_AND) || (reducedRule == RULE_OR)))
       {
           EdgeLabel root1 = 0;
           packRule(root1, RULE_X);
       ...
       if (isRuleAH(incomingRule) && ((reducedRule == RULE_AND) || (reducedRule == RULE_OR)))
       {
           EdgeLabel root1 = 0;
           packRule(root1, RULE_X);

           child[0] = mergeEdge(beginLevel, mergeLevel - 1, root1, reduced);
           child[0].setComp(child[1].getComp());
           child[1].setRule(RULE_X);
           if (incomingRule == RULE_AL0) {
               child[1].setEdgeHandle(makeTerminal(INT, 0));
           } else {
    ...
    *
    * --------------------------------------------------------------------------------------------*/
    if ((reducedRule == RULE_X) && ((incomingRule == RULE_OR) || (incomingRule == RULE_AND))) {
        return normalizeEdge(beginLevel, reduced);
    }
...
Edge Forest::reduceEdge(const Level beginLevel, const EdgeLabel label, const Level nodeLevel, const std::vector<Edge>& down, const Value& value)
{
    ...
    if (setting.hasReductionRule(RULE_AND) && setting.hasReductionRule(RULE_OR)) {
        if (unpackSwap(label)) {
            std::cout << "Label has swap!!" << std::endl;
            exit(0);
        } if (unpackComp(label)) {
            std::cout << "Label has comp!!" << std::endl;
            exit(0);
        }
        // This is ugly but we gotta do it ig to not save node
        EdgeLabel mergeLabel = 0;
        packRule(mergeLabel, unpackRule(label));

        if (beginLevel < nodeLevel) {
            std::cout << "[BRAVE_DD] ERROR\t reduceEdge(): Invalid for incoming edge ro target node!" << std::endl;
        }

        reduced = reduceNode(nodeLevel, child);

        if (unpackSwap(label) && unpackRule(label) == RULE_OR)
            packRule(mergeLabel, RULE_AND);
        else if (unpackSwap(label) && unpackRule(label) == RULE_AND)
            packRule(mergeLabel, RULE_OR);
