                    ...
                    int start, int end)
{
    ...
    if (lvl == 1) {
        ...
        std::cout << "start " << start << "fun[start] " << fun[start] << std::endl; 
        ...
        std::cout << "end " << end << "fun[end] " << fun[end] << std::endl;
        child[0].setRule(RULE_X); 
        child[1].setRule(RULE_X);
        return forest->reduceEdge(lvl, label, lvl, child);
    }
...
bool buildEvSetForest(uint16_t num, PredefForest bdd)
{
    ...
    vars[0] = std::vector<bool>{1,0,0,1};

    // function values
    Func function(forest);
    std::vector<std::vector<int> > funs(1, std::vector<int>(1<<num, false));
    funs[0] = std::vector<int>{0,1,0,0,0,0,0,0};
    for (int i=0; i < 1; i++) { 
        // build function
        Edge edge = buildEvSetEdge(forest, num, funs[0], 0, (1<<num)-1);
        function.setEdge(edge);
        // evaluate function
        // for (int j=0; j<(1<<num); j++) {
        for (int j=0; j < 1; j++) {
