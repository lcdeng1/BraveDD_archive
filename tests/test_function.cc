...
bool buildEvSetForest(uint16_t num, PredefForest bdd)
{
    ...
    funs[0] = std::vector<int>{1, 1, 3, 1, 1, 5, 5 ,3, 3, 5, 5, 3, 5, 1, 3, 3};
    for (int i = 0; i< 16; i++) {
        funs[0][i] = funs[0][i] % setting.getMaxRange();
    }
    for (int i=0; i < 1; i++) { 
        // build function
        ...
        for (int j=0; j<1; j++) {
            ...
            std::string name = bdd == PredefForest::EVMODQBDD ? "EVQ" : "EVF";
...
bool buildEvQSetForest(uint16_t num, PredefForest bdd)
{
    // test up to 5 variables
    if (num > 5) return 0;
    ForestSetting setting(bdd, num);
    ...
    Func function(forest);
    std::vector<std::vector<int> > funs(1, std::vector<int>(1<<num, false));
    funs[0] = std::vector<int>{2, 1, 4, 1, 4, 1, 3, 1, 4, 5, 1, 1, 4, 4, 1, 0};
    for (int i=0; i < 1; i++) { 
        // build function
        Edge edge = buildEvSetEdge(forest, num, funs[i], 0, (1<<num)-1);
        function.setEdge(edge);
...
int main(int argc, char** argv)
{
    ...
    // buildEvSetForest(4, PredefForest::EVMODQBDD);
    buildEvSetForest(4, PredefForest::EVMODFBDD);
    // buildEvQSetForest(4, PredefForest::EVQBDD);
}