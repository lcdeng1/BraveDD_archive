...
inline void show_elapsed(const char* what, const char* avg, long time, unsigned nmt)
{
  ...
  fprintf(stderr, "%s %ld.%06ld seconds\n", what,
    elapsed_sec, elapsed_usec);
  fprintf(stderr, "%s took %Lf microseconds for %u minterms\n", avg,
    res/nmt*1000000, nmt);
}
...
int main(int argc, const char** argv)
{
    ...
    bool swap;
	short shift;
    BDD.add_nodes_from_file(argv[2], root, swap, shift);
		BDD.done_building();
...
//    std::cerr << "Got root " << root << "\n";
    ...
    for (unsigned i = 0;i<nmt;i++) {
        if (get_query(min_arr[i], vars, BDD.num_levels()))
			    {
			      show_query(std::cout, vars, BDD.num_levels(), BDD.evaluate(root, swap, shift, vars));
			    }
    }
