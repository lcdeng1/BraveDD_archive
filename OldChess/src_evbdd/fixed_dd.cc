...
unsigned* fixed_dd::add_nodes_from_file(const char* pathname)
{
  ...
  if (zcat) {
    ...
    if ( ('E' != buffer[0]) or ('V' != buffer[1]) or ('B' != buffer[2]) or ('D' != buffer[3]) or ('D' != buffer[4]) or ('\n' != buffer[5]) ) {
			BDD_READER_TERMINATE("Not a BDD file");
		}

    //
    // Input order L, T, N, R, S
    //
    expect_char(fin, 'L');
...
#ifdef WITH_SWAPS
    expect_char(fin, 'S');
    bool file_swapped = expect_unsigned(fin);
#ifdef DEBUG_PARSER
    std::printf("Got R %u\n", file_roots);
#endif
    ...
    for (k=1; k<=file_levels; k++) {
      ...
      for (; nk; --nk) {
        ...
        char low_edge = expect_unsigned(fin);
#ifdef WITH_SWAPS
        bool low_swap = expect_unsigned(fin) ^ file2swap[child];
#else
        bool low_swap = false;
#endif
        ...
        char high_edge = expect_unsigned(fin);
#ifdef WITH_SWAPS
        bool high_swap = expect_unsigned(fin) ^ file2swap[child];
#else
...
#ifdef WITH_SWAPS
        if (file_swapped){
            if ((low > high) || ((low == high) && (low_edge > high_edge)) || ((low_edge == high_edge) && (low == high) && (low_swap == true))) {
...
unsigned* fixed_dd::evaluate(unsigned f_root, const bool* vars, unsigned f_val, bool f_swap) const
{
    bool dir;
  ...
  unsigned tmp = f_root;
  for (;;) {
      if (node_level[f_root] == 0) { //Terminal node: compute value
          val[1] = (f_root == 0);
          val[0] = (f_root == 0) ? 0 : (val[0] + f_val);
          return val;
      }
      ...
      if (dir == (edge_value[tmp] >= power(2, 7))) {
          val[0] += edge_value[tmp] % power(2, 7);
      }
...
inline unsigned hashnode(unsigned char level, unsigned child0, unsigned child1, unsigned edge)
{
  unsigned H = 0x480bdd;
  hashbyte(H, level);
  hashword(H, child0);
...
unsigned fixed_dd::add_unique(unsigned char level, unsigned child0, unsigned child1, char edge0, char edge1, bool swap0, bool swap1)
{
  ...
  char edge;
  if (swap0) child0 += power(2, 31);
  if (swap1) child1 += power(2, 31);
  if (edge0 != 0) edge = edge0;
  else edge = edge1 + power(2, 7);

  // Hash node
  unsigned H = hashnode(level, child0, child1, edge) % max_nodes;
  ...
  for (unsigned curr = ut[H]; curr; curr = ut_next[curr]) {
    ...
    if (edge_value[curr] != edge)  continue;
    return curr;
  }
...
void fixed_dd::enlarge_ut(unsigned req_max)
{
  ...
  edge_value = static_cast <char*> (std::realloc(edge_value, rm * sizeof(unsigned)));
  if (nullptr == edge_value) {
  ...
  for (u=terminals; u<next_node; u++) {
    ...
    unsigned H = hashnode(node_level[u], node_child0[u], node_child1[u], edge_value[u]) % rm;
    ut_next[u] = ut[H];
    ut[H] = u;
  }
