  ...
  public:
    ...
    */
    ...
    inline char evaluate(bdd_edge root, const char* minterm) const
    {
        ...
        for (;;) {
            val += root.edge;
            if (root.node < get_num_terminals()) {
                if (root.node == 0) 
                    return '0';
                else 
                    return terminal2char(val + 1);
            }
            const bdd_node& node = node_array[root.node];
            const char var = minterm[node.level];
            ASSERT('0' == var || '1' == var);
    ...
    */
    ...
    /* 
    * Given a node handle, return the node's low edge.
    */
...
#ifdef WITH_SWAPS
    /*
    * Given a node handle, return the node's low swap.
    */
    ...
    /**
      Given a node handle, return the node's high swap.
    */
    inline bool node_swap1(unsigned handle) const
    {
        ASSERT(handle < next_node);
    ...
    */
    ...
    //void write_info(const char* fileName, std::ostream& out, unsigned* rts, unsigned nr) const;
