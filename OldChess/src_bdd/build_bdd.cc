...
struct switches {
  ...
  char use_shifters;        // use variable shifters when encodign?
...
static void build_mt(parser* P, const switches &S, bdd_forest &bdd, compute_table &CT, unsigned &root)
{
...
static void concretize(const switches &S, bdd_forest &bdd, compute_table &CT, unsigned* roots, unsigned nroots)
{
  long start_time = get_time_in_us();
...
// **********************************************************************
//
// APPLY VARIABLE SHITERS TO MTBDD
// *To toggle this functionality, change #define WITH_SHIFTS in
// bdd.h and fixed_dd.h
//
// **********************************************************************

#ifdef WITH_SHIFTS
inline void apply_shifts(bdd_forest& bdd, unsigned* roots, short* shifts, unsigned nroots) {
    long start_time = get_time_in_us();

    fprintf(stderr, "Applying variable shifters...\n");

    bdd.apply_shift(roots, shifts, nroots);
    unsigned count = bdd.mark_and_sweep(roots, nroots);

    show_elapsed("Swap bit application", get_time_in_us() - start_time);

    report(count, "nodes in shifted-BDD\n");
    report(count * sizeof(bdd_node), "bytes for shifted-BDD\n");
    fflush(stderr);
}
#endif
...
#ifdef WITH_SWAPS
inline void apply_swap(bdd_forest& bdd, unsigned* roots, bool* swaps, unsigned nroots) {
    long start_time = get_time_in_us();

    fprintf(stderr, "Applying swap bit...\n");

    bdd.apply_swap(roots, swaps, nroots);
    unsigned count = bdd.mark_and_sweep(roots, nroots);
...
static bool verify_mt(const bdd_forest &bdd, unsigned shift, unsigned root, unsigned swap, const char* vfile)
{
  ...
  for (;;) {
    ...
    char term = bdd.evaluate(shift, swap, root, minterm);
    if (value != term) {
...
static void build_bw(parser* P, const switches &S, bdd_forest &bdd, unsigned roots[], unsigned nroots)
{
  char* linebuf = new char[5+bdd.get_num_vars()];
...
static void init_switches(optman &OM, switches &S)
{
    ...
    OM.add_option('s', new letter_option(
        S.use_swap_bit, "Use swap bit on edges. \n"
                        "Canonical by setting low edge to\n"
                        "lower node in UT. If both edges point\n"
                        "to same node, low edge has swap=false.\n"
                        "Implimented after concretization."
    ));
    S.use_swap_bit = 0;
#endif

    //USE VARIABLE SHIFTERS IF ALLOWED
#ifdef WITH_SHIFTS
    // -s
    OM.add_option('S', new letter_option(
        S.use_shifters, "Use variable shifters on edges. \n"
        "Implimented after concretization,\n"
        "but before swap bits"
    ));
    S.use_shifters = 0;
#endif
...
int main(int argc, const char** argv)
{
  try {
    ...
    BDD.set_CT(&CT);
		...
		unsigned* roots = new unsigned[1+outbits];
        for (unsigned i = 0; i <= outbits; ++i) { roots[i] = 0; }
        bool* swaps = new bool[1 + outbits];
        short* shifts = new short[1 + outbits];
        for (unsigned i = 0; i <= outbits; ++i) { swaps[i] = false; }
    ...
    //
        ...
        //
        //Encode with variable shifting
        //
#ifdef WITH_SHIFTS
        if (S.use_shifters) {
            if (MTERM == S.encoding) {
                apply_shifts(BDD, roots, shifts, outbits + 1);
            }
        }
#endif
    ...
    //
        ...
        if (S.use_swap_bit) {
            if (MTERM == S.encoding) {
                apply_swap(BDD, roots, swaps, outbits + 1);
            }
        }
#endif
    ...
    //
		if (MTERM == S.encoding) {
			if (!verify_mt(BDD, shifts[0], roots[0], swaps[0], S.verify_file)) {
				return 1;
			}
		}
    ...
    if (S.write_file) {
      ...
      if (!outf) {
        fprintf(stderr, "Couldn't write to file %s\n", S.write_file);
              ...
              if (MTERM == S.encoding) {
                  BDD.write_exch(outf, roots[0], swaps[0], shifts[0]);
              }
              else {
                  BDD.write_exch(outf, roots, swaps, shifts, 1 + outbits);
              }
          }
      }
    }
