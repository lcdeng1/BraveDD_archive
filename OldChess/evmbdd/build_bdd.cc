...
{
  char* linebuf = new char[bdd.get_mod_val() + bdd.get_num_vars()];
...
static void init_switches(optman &OM, switches &S)
{
    ...
    OM.add_option('s', new letter_option(
        ...
        "Canonical by setting low edge's edge value to 0.\n"
        "The low edge also points to the lower node. If both edges point to the same node\n"
        "then the low edge should have swap=false. If both edges point to the same node and\n"
        "have swap=false, then the incoming edge has swap=false."
    ));
    S.use_swap_bit = 0;
#endif
