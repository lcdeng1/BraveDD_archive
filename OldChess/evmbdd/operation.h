...
class operation {
  ...
  protected:
    ...
    inline bdd_edge newEdge(char edge, unsigned node) {
      return BDD.newEdge(edge, node);
    }

    inline bdd_edge normalize(bdd_node N)
    {
      return BDD.Normalize(N);
    }
    ...
    inline unsigned node_edge0(unsigned handle) const
    {
      return BDD.node_edge0(handle);
    }

    inline unsigned node_edge1(unsigned handle) const
    {
      return BDD.node_edge1(handle);
    }

    inline const bdd_node& get_node(unsigned handle) const 
    {
      return BDD.get_node(handle);
    }
