...
unsigned bdd_forest::add_node(const bdd_node &NN)
{
  ...
  if (!((N.child[0].edge == 0 && N.child[0].node != 0) || (N.child[1].edge == 0 && N.child[1].node != 0) || (N.child[0].node == 0 && N.child[1].node == 0)) ){
      std::cout << int(N.child[0].node) << " " << int(N.child[0].edge) << " " << bool(N.child[0].swap) << "\t|\t" << int(N.child[1].node) << " " << int(N.child[1].edge) << " " << bool(N.child[1].swap) << "\n";
  }
...
/*unsigned bdd_forest::reduce(unsigned root) {
    ...
    bdd_node& current_node = node_array[root];
    current_node.child[0].node = reduce(current_node.child[0].node);
    current_node.child[1].node = reduce(current_node.child[1].node);

    if ((current_node.child[0].node == current_node.child[1].node) && (current_node.child[0].edge == 0) && (current_node.child[1].edge == 0))
        return current_node.child[0].node;
...
bdd_edge bdd_forest::Normalize(bdd_node u) {
  char r = 0;
  if (u.child[0].node != 0 && u.child[0].edge != 0) {
    ASSERT(u.child[0].edge >= 0 && u.child[1].edge >= 0);
    r = u.child[0].edge;
    u.child[0].edge = 0;
    if (u.child[1].node != 0) {
      u.child[1].edge = ((u.child[1].edge - r + modVal) % modVal);
      ASSERT(u.child[1].edge >= 0);
    }
  } else if (u.child[0].node == 0) {
    if (u.child[1].node != 0) {
      r = u.child[1].edge;
      u.child[1].edge = 0;
    } else {
      // this is the case that both child edge of u are infty, then return the long infty edge
      return newEdge(0,0);
    }
  }
  return newEdge(r, add_node(u));
}
...
bdd_edge bdd_forest::apply_swap_rec(bdd_edge& root) {
    ...
    if (tmp.child[0].node == 0) {
        pushVal = tmp.child[1].edge;
        tmp.child[0].edge = 0;
        tmp.child[1].edge = 0;
    }
    ...
    if (tmp.child[0].node > tmp.child[1].node) {
        s = true;
    } else if ((tmp.child[0].node == tmp.child[1].node) && (tmp.child[0].swap > tmp.child[1].swap)) {
        s = true;
    } else if ((tmp.child[0].node == tmp.child[1].node) && (tmp.child[0].swap == tmp.child[1].swap) && (tmp.child[1].edge > modVal/2)) {
        s = true;
    }

    //Swap
    if (s) {
        char new_root_edge = (root.edge + tmp.child[1].edge + pushVal) % modVal;
        unsigned tmpNode0 = tmp.child[0].node;
        unsigned tmpNode1 = tmp.child[1].node;
        bool tmpSwap0 = tmp.child[0].swap;
        bool tmpSwap1 = tmp.child[1].swap;

        tmp.child[0].node = tmpNode1;
        tmp.child[1].node = tmpNode0;
        tmp.child[0].swap = tmpSwap1;
        tmp.child[1].swap = tmpSwap0;

        tmp.child[1].edge = ((tmp.child[0].edge + root.edge + pushVal) - new_root_edge + modVal) % modVal;
        tmp.child[0].edge = 0;
        

        return newEdge(new_root_edge, add_node(tmp), true);
    }

    return newEdge((root.edge + pushVal) % modVal, add_node(tmp), false);
}
#endif


//UnionMin Algorithm for merging EV+modBDD Base cases
bdd_edge bdd_forest::UnionMin(bdd_edge edge_1, bdd_edge edge_2) {
  ...
  if (edge_1.edge == edge_2.edge && edge_1.node == edge_2.node) return newEdge(edge_1.edge, edge_1.node);
  if (edge_1.node == edge_2.node && edge_1.node == 1) return newEdge(min(edge_1.edge, edge_2.edge), 1);
  ...
  bool f1 = (node_level(edge_1.node) == node_level(edge_2.node) || node_level(edge_1.node) > node_level(edge_2.node));
  bool f2 = (node_level(edge_1.node) == node_level(edge_2.node) || node_level(edge_1.node) < node_level(edge_2.node));
  
  // recursion for low edge
  a = (edge_1.edge + (f1? node_edge0(edge_1.node) : 0)) % modVal;
  b = (edge_2.edge + (f2? node_edge0(edge_2.node) : 0)) % modVal;
  
  ue1 = newEdge(a, f1? node_child0(edge_1.node) : edge_1.node);
  ue2 = newEdge(b, f2? node_child0(edge_2.node) : edge_2.node);

  u_node.child[0] = UnionMin(ue1, ue2);

  // recursion for high edge
  a = (edge_1.edge + (f1? node_edge1(edge_1.node) : 0)) % modVal;
  b = (edge_2.edge + (f2? node_edge1(edge_2.node) : 0)) % modVal;
  
  ue1 = newEdge(a, f1? node_child1(edge_1.node) : edge_1.node);
  ue2 = newEdge(b, f2? node_child1(edge_2.node) : edge_2.node);
  
  u_node.child[1] = UnionMin(ue1, ue2);

  // normalize
  ASSERT(u_node.child[0].edge >= 0 && u_node.child[1].edge >= 0);

  return Normalize(u_node);
}
