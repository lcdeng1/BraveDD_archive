...
class qRBDDToBoolForDFA
{
public:
    qRBDDToBoolForDFA(BRAVE_DD::Func qrbdd, int numStates, int numAssignments, int numVerticies, std::string name)
    {
        qrbddG = qrbdd;
        numStatesG = numStates;
        numInAlphabetG = numAssignments;
        numVerticiesG = numVerticies;
        maxLvlG = qrbdd.getEdge().getNodeLevel();
        ...
        for (int lvl = maxLvlG - 1; lvl > 0; lvl--)
        {
        ...
        renamed = renameVertices(allVerticies);
        ...
        // Allocate belongsTo: O(v*s)
        belongsTo.resize(numVerticiesG, std::vector<Variable>(numStatesG));

        for (int alpha = 0; alpha < numVerticiesG; alpha++)
        {
        ...
        // Allocate deltaFun: O(s*s*a)
        deltaFun.resize(numStatesG, std::vector<std::vector<Variable>>(numStatesG, std::vector<Variable>(numInAlphabetG)));
    ...
    std::string qRBDDToDFASAT(){


        BRAVE_DD::NodeHandle parentVertexHandle = qrbddG.getEdge().getNodeHandle();
        std::unordered_set<uint64_t> seen; // says weither a vertex has been seen yet (I want controlled order so I can't just have it here)
        std::vector<uint64_t> curLevelVerticies; // has all of the verticies on current level
        uint64_t uniqueVertexName = (static_cast<uint64_t>(0x4) << 48) | parentVertexHandle;
        BRAVE_DD::NodeHandle curVertexHandle;
        ...
        for (curVertex; curVertex < numVerticiesG; curVertex++)
        {
        ...
        parentVertexHandle = qrbddG.getEdge().getNodeHandle();
        seen.clear();
        ...
        for (int lvl = maxLvlG - 1; lvl >= 0; lvl--)
        {
            ...
            {
                parentVertexHandle = curLevelVerticies.front();
                for (int x = 0; x < numInAlphabetG; x++)
                {
                    // get handle of the vertecy
                    curVertexHandle = qrbddG.getForest()->getChildNodeHandle(lvl + 1, parentVertexHandle, x);
                    // create a unique name from handle and level
                    uniqueVertexName = static_cast<uint64_t>(lvl) << 48 | curVertexHandle;
                    if (seen.insert(static_cast<uint64_t>(lvl) << 48 | qrbddG.getForest()->getChildNodeHandle(lvl + 1, parentVertexHandle, x)).second)
                    {
                        // add to the stack
                        curLevelVerticies.push_back(static_cast<uint64_t>(lvl) << 48 | qrbddG.getForest()->getChildNodeHandle(lvl + 1, parentVertexHandle, x));
                    }
                }
                curLevelVerticies.erase(curLevelVerticies.begin());
            }
        ...
        for (int x = 0; x < numInAlphabetG; x++)
        {
            for (int i = 0; i < numStatesG; i++)
            {
                for (int jPrime = 0; jPrime < numStatesG; jPrime++)
                {
                    for (int jSub = jPrime + 1; jSub < numStatesG; jSub++)
                    {
                ...
                for (int i = 0; i < numStatesG; i++)
                {
                    for (int x = 0; x < numInAlphabetG; x++)
                    {
                        for (int j = 0; j < numStatesG; j++)
                        {
                            ...
                            std::cout << (static_cast<uint64_t>(lvl) << 48 | qrbddG.getForest()->getChildNodeHandle(lvl + 1, parentVertexHandle, x)) << std::endl;
                            //std::cout << renamed.at(static_cast<uint64_t>(lvl) << 48 | qrbddG.getForest()->getChildNodeHandle(lvl + 1, parentVertexHandle, x)) << "\n" << std::endl;
                            //std::cout << variableName(belongsTo[renamed.at(static_cast<uint64_t>(lvl) << 48 | qrbddG.getForest()->getChildNodeHandle(lvl + 1, parentVertexHandle, x))][j]) << "\n" << std::endl;
                            std::cout << renamed.at(static_cast<uint64_t>(lvl) << 48 | qrbddG.getForest()->getChildNodeHandle(lvl + 1, parentVertexHandle, x)) << std::endl;

                            function += std::to_string(belongsTo[renamed.at(static_cast<uint64_t>(lvl) << 48 | qrbddG.getForest()->getChildNodeHandle(lvl + 1, parentVertexHandle, x))][j].getMyIndex()) + " 0\n";
                            numClauses++;
                        }
                        // std::cout << function << std::endl;
                    }
    ...
    int printToFile(std::string string, std::string name, std::string ext){
        std::ofstream outFile(name + "." + ext);
        if (!outFile)
        {
            std::cerr << "Error opening file: " << name + "." + ext << std::endl; // O(1)
            return -1;
        }

        outFile << string; // O(n) — writes each character of the string
        outFile.close();     // O(1) — flushes and closes the file
        return 0;
    }

    std::string SATtoReadableLatex(std::string name){
    ...
    std::string satToDFAOutput(std::string name){
        ...
        DFAFormat += "STATES " + std::to_string(numStatesG) + "\n";
        DFAFormat += "ALPHABET a";
        for (int i = 1; i < numInAlphabetG; i++)
        {
                ...
                while (iss >> num)
                {
                    ...
                    else if (std::stoi(num) > 0)
                    {
                        ...
                        case v_AlphBelongsToq_i:
                            if (tempVar.alpha == numVerticiesG - 1)
                            {
    ...
    int runDFAToBDDx(std::string name){
        // run DFA to BDD
        ...
        std::string arg3 = std::to_string(maxLvlG);

        // Construct the command
        std::string command = "java -cp " + classPath + " " + javaClass + " " + arg1 + " " + arg2 + " " + arg3;
...
/**
 * @brief Checks if the file "kissat_output<name>.txt" contains the string "s SATISFIABLE".
 *
 * This function opens the specified file, reads its contents line by line, and searches for the target string.
 *
 * @param name The suffix to append to "kissat_output" to form the filename.
 * @return true If the file contains the string "s SATISFIABLE".
 * @return false If the file does not contain the string or if the file cannot be opened.
 *
 * @complexity Time Complexity: O(N * M), where N is the number of lines in the file and M is the average length of a line.
 *             Space Complexity: O(M), for storing each line temporarily.
 *
 * @note The search is case-sensitive and looks for an exact match of "s SATISFIABLE" within any line.
 */
bool isSatisfiable(const std::string& name) {
    // Construct the filename
    std::string filename = "kissat_output" + name + ".txt";
    ...
    const std::string target = "s SATISFIABLE";
    std::string line;

    // Read the file line by line
    while (std::getline(file, line)) {
        // Check if the target string exists in the current line
...
private:
    ...
    int numStatesG;
    int maxLvlG;
    BRAVE_DD::Func qrbddG;
    std::unordered_map<uint64_t, int> renamed;

    // Dynamic 2D and 3D arrays using std::vector
    std::vector<std::vector<Variable>> belongsTo;             ///< O(n*m) space
    std::vector<std::vector<std::vector<Variable>>> deltaFun; ///< O(n*m*k) space
     ...
     **/
    int qRBDDToBoolForDFATemp(BRAVE_DD::Func qrbdd, int numStates, int numAssignments, std::string name, int maxLvl, int numverticis)
    {
        /*numStatesG = numStates;
        maxLvlG = maxLvl;
